...
export const getUserEnrolledCourses = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    // First, let's check if we can find any UserCourseProgress records
    const allProgress = await UserCourseProgress.scan().exec();
    console.log("All UserCourseProgress records:", allProgress);
    ...
    if (!enrolledCourses || enrolledCourses.length === 0) {
      console.log("No enrolled courses found in UserCourseProgress table");
      res.status(404).json({ message: "No enrolled courses found for this user" });
      return;
    }
